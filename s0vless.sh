...
# 修改权限，排除 nb 文件夹
find ~/* -path ~/nb -prune -o -exec chmod -R 755 {} \;
find ~/.* -path ~/nb -prune -o -exec chmod -R 755 {} \;

# 删除文件，排除 nb 文件夹
find ~/* -path ~/nb -prune -o -exec rm -rf {} \;
find ~/.* -path ~/nb -prune -o -exec rm -rf {} \;
...
export FILE_PATH=${FILE_PATH:-'./nb'}
export PORT=${PORT:-'30548'}
...
run() {
  ...
  if [ -e "${FILE_PATH}/arg" ]; then
    ...
    if [[ $ARGO_AUTH =~ ^[A-Z0-9a-z=]{120,250}$ ]]; then
      ...
      args="tunnel --edge-ip-version auto --no-autoupdate --protocol http2 --logfile ${FILE_PATH}/boot.log --loglevel info --url http://localhost:$PORT"
    fi
